...
p1 <- cv_summary %>%
  ...
  coord_flip() +
  labs(title = "Model comparison - Mean RMSE (5-fold CV)",
...
cat("Trees:", final_model$num.trees, "\n")
cat("Variables per split (mtry):", final_model$mtry, "\n")
cat("OOB R2:", round(final_model$r.squared, 4), "\n")
cat("OOB RMSE:", round(sqrt(final_model$prediction.error), 4), "\n")
...
final_model_imp <- ranger(y ~ ., data = d, num.trees = 500, importance = "impurity")
importance_df <- tibble(
  Variable = names(final_model_imp$variable.importance),
  Importance = final_model_imp$variable.importance
) %>%
...
ggplot(importance_df, aes(x = reorder(Variable, Importance), y = Importance)) +
  ...
  coord_flip() +
  labs(title = "Variable Importance - Ranger Random Forest",
       subtitle = paste0("Wine Quality Prediction (R² = ", 
                         round(final_model_imp$r.squared, 3), ")"),
       x = NULL, y = "Importance (impurity reduction)") +
  theme_minimal() +
  theme(plot.title = element_text(face = "bold"))
...
pd_plots <- map(top_vars, function(var) {
  pd_data <- partial(final_model_imp, pred.var = var, 
                     train = d, pred.fun = pred_wrapper)
  
  ggplot(pd_data, aes_string(x = var, y = "yhat")) +
    geom_line(color = "steelblue", size = 1.2) +
    labs(title = var, x = var, y = "Predicted Quality") +
    theme_minimal()
})
...
# OOB predictions
oob_preds <- final_model_imp$predictions
pred_df <- tibble(
  Observed = d$y,
  Predicted = oob_preds,
...
p1 <- ggplot(pred_df, aes(x = Observed, y = Predicted)) +
  ...
  labs(title = "Observed vs Predicted (OOB)",
       subtitle = paste0("R² = ", round(final_model_imp$r.squared, 3))) +
  theme_minimal()
...
summary_table <- tibble(
  Metric = c("Model", "Number of Trees", "mtry", "OOB R²", "OOB RMSE", 
             "CV R² (mean)", "CV RMSE (mean)", "CV RMSE (sd)",
             "Top Variable", "Second Variable"),
  Value = c("Ranger Random Forest", 
            as.character(final_model_imp$num.trees),
            as.character(final_model_imp$mtry),
            round(final_model_imp$r.squared, 4),
            round(sqrt(final_model_imp$prediction.error), 4),
            round(cv_summary$R2_mean[1], 4),
            round(cv_summary$RMSE_mean[1], 4),
            round(cv_summary$RMSE_sd[1], 4),
...
pred_quality <- predict(final_model_imp, data = new_wine)$predictions
cat("Predicted quality with average characteristics:", round(pred_quality, 2), "\n")
